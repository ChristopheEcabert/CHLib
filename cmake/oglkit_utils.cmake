...
macro(DISSECT_VERSION)
    ...
    string(REGEX REPLACE "^([0-9]+).*" "\\1"
        OGLKIT_MAJOR_VERSION "${OGLKIT_VERSION}")
    string(REGEX REPLACE "^[0-9]+\\.([0-9]+).*" "\\1"
        OGLKIT_MINOR_VERSION "${OGLKIT_VERSION}")
    string(REGEX REPLACE "^[0-9]+\\.[0-9]+\\.([0-9]+).*" "\\1"
        OGLKIT_REVISION_VERSION "${OGLKIT_VERSION}")
    string(REGEX REPLACE "^[0-9]+\\.[0-9]+\\.[0-9]+(.*)" "\\1"
        OGLKIT_CANDIDATE_VERSION "${OGLKIT_VERSION}")
  ...
  if (NOT DEFINED LIB_INSTALL_DIR)
    ...
    if(NOT ANDROID)
      set(INCLUDE_INSTALL_ROOT
          "include/${PROJECT_NAME_LOWER}-${OGLKIT_MAJOR_VERSION}.${OGLKIT_MINOR_VERSION}")
    else(NOT ANDROID)
      set(INCLUDE_INSTALL_ROOT "include") # Android, don't put into subdir
    endif(NOT ANDROID)
    set(INCLUDE_INSTALL_DIR "${INCLUDE_INSTALL_ROOT}/pcl")
    set(DOC_INSTALL_DIR "share/doc/${PROJECT_NAME_LOWER}-${OGLKIT_MAJOR_VERSION}.${OGLKIT_MINOR_VERSION}")
    set(BIN_INSTALL_DIR "bin")
    set(PKGCFG_INSTALL_DIR "${LIB_INSTALL_DIR}/pkgconfig")
    if(WIN32 AND NOT MINGW)
        set(OGLKITCONFIG_INSTALL_DIR "cmake")
      else(WIN32 AND NOT MINGW)
        set(OGLKITCONFIG_INSTALL_DIR "share/${PROJECT_NAME_LOWER}-${OGLKIT_MAJOR_VERSION}.${OGLKIT_MINOR_VERSION}")
...
###############################################################################
# Helper function for CUDA architecture detection 
# From https://github.com/BVLC/caffe/blob/6eae122a8eb84f8371dde815986cd7524fc4cbaa/cmake/Cuda.cmake#L9-L132

# Known NVIDIA GPU achitectures Caffe can be compiled for.
# This list will be used for CUDA_ARCH_NAME = All option
set(OGLKIT_KNOWN_GPU_ARCHS "20 21(20) 30 35 50")

###############################################################################
# A function for automatic detection of GPUs installed  (if autodetection is enabled)
# _detected_gpu   Variable set with output of the detection
# Usage:
#   OGLKIT_DETECT_GPUS(_detected_gpu)
function(OGLKIT_DETECT_GPUS _detected_gpu)
  # Is there some detection ?
  if(NOT CUDA_GPU_DETECT_OUTPUT)
    set(__cufile ${OGLKIT_BINARY_DIR}/detect_cuda_archs.cu)
  ...
  endif(NOT CUDA_GPU_DETECT_OUTPUT)

  if(NOT CUDA_GPU_DETECT_OUTPUT)
    message(STATUS "Automatic GPU detection failed. Building for all known architectures.")
    set(${_detected_gpu} ${OGLKIT_KNOWN_GPU_ARCHS} PARENT_SCOPE)
  else(NOT CUDA_GPU_DETECT_OUTPUT)
    set(${_detected_gpu} ${CUDA_GPU_DETECT_OUTPUT} PARENT_SCOPE)
  endif(NOT CUDA_GPU_DETECT_OUTPUT)
endfunction()

################################################################################################
# Function for selecting GPU arch flags for nvcc based on CUDA_ARCH_NAME
# _nvcc_flags   Automatic flags detection for arch/code flags
# Usage:
#   OGLKIT_SELECT_NVCC_ARCH_FLAGS(_nvcc_flags)
function(OGLKIT_SELECT_NVCC_ARCH_FLAGS _nvcc_flags)
  # List of arch names
  ...
  if(${CUDA_ARCH_NAME} STREQUAL "Manual")
    set(CUDA_ARCH_BIN ${OGLKIT_KNOWN_GPU_ARCHS} CACHE STRING "Specify 'real' GPU architectures to build binaries for, BIN(PTX) format is supported")
  ...
  elseif(${CUDA_ARCH_NAME} STREQUAL "All")
    set(__cuda_arch_bin ${OGLKIT_KNOWN_GPU_ARCHS})
  elseif(${CUDA_ARCH_NAME} STREQUAL "Auto")
    OGLKIT_DETECT_GPUS(__cuda_arch_bin)
  ...
  set(${_nvcc_flags}          ${__nvcc_flags}          PARENT_SCOPE)
  set(${_nvcc_flags}_readable ${__nvcc_archs_readable} PARENT_SCOPE)
endfunction()