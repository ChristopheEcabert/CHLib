include(${PROJECT_SOURCE_DIR}/cmake/chlib_utils.cmake)
...
macro(CHLIB_SUBSYS_OPTION _var _name _desc _default)
    set(_opt_name "BUILD_${_name}")
    CHLIB_GET_SUBSYS_HYPERSTATUS(subsys_status ${_name})
    if(NOT ("${subsys_status}" STREQUAL "AUTO_OFF"))
      ...
      if((NOT ${_default} AND NOT ${_opt_name}) OR ("${_default}" STREQUAL "AUTO_OFF"))
        ...
        CHLIB_SET_SUBSYS_STATUS(${_name} FALSE ${_reason})
        CHLIB_DISABLE_DEPENDIES(${_name})
      elseif(NOT ${_opt_name})
        set(${_var} FALSE)
        CHLIB_SET_SUBSYS_STATUS(${_name} FALSE "Disabled manually.")
        CHLIB_DISABLE_DEPENDIES(${_name})
      else(NOT ${_default} AND NOT ${_opt_name})
        set(${_var} TRUE)
        CHLIB_SET_SUBSYS_STATUS(${_name} TRUE)
        CHLIB_ENABLE_DEPENDIES(${_name})
      endif((NOT ${_default} AND NOT ${_opt_name}) OR ("${_default}" STREQUAL "AUTO_OFF"))
    endif(NOT ("${subsys_status}" STREQUAL "AUTO_OFF"))
    CHLIB_ADD_SUBSYSTEM(${_name} ${_desc})
endmacro(CHLIB_SUBSYS_OPTION)
...
# _default The default value (TRUE or FALSE)
# ARGV5 The reason for disabling if the default is FALSE.
macro(CHLIB_SUBSUBSYS_OPTION _var _parent _name _desc _default)
  set(_opt_name "BUILD_${_parent}_${_name}")
  CHLIB_GET_SUBSYS_HYPERSTATUS(parent_status ${_parent})
  if(NOT ("${parent_status}" STREQUAL "AUTO_OFF") AND NOT ("${parent_status}" STREQUAL "OFF"))
    CHLIB_GET_SUBSYS_HYPERSTATUS(subsys_status ${_parent}_${_name})
    if(NOT ("${subsys_status}" STREQUAL "AUTO_OFF"))
      option(${_opt_name} ${_desc} ${_default})
      if((NOT ${_default} AND NOT ${_opt_name}) OR ("${_default}" STREQUAL "AUTO_OFF"))
        set(${_var} FALSE)
        ...
        CHLIB_SET_SUBSYS_STATUS(${_parent}_${_name} FALSE ${_reason})
        CHLIB_DISABLE_DEPENDIES(${_parent}_${_name})
      elseif(NOT ${_opt_name})
        set(${_var} FALSE)
        CHLIB_SET_SUBSYS_STATUS(${_parent}_${_name} FALSE "Disabled manually.")
        CHLIB_DISABLE_DEPENDIES(${_parent}_${_name})
      else(NOT ${_default} AND NOT ${_opt_name})
        set(${_var} TRUE)
        CHLIB_SET_SUBSYS_STATUS(${_parent}_${_name} TRUE)
        CHLIB_ENABLE_DEPENDIES(${_parent}_${_name})
      endif((NOT ${_default} AND NOT ${_opt_name}) OR ("${_default}" STREQUAL "AUTO_OFF"))
    endif(NOT ("${subsys_status}" STREQUAL "AUTO_OFF"))
  endif(NOT ("${parent_status}" STREQUAL "AUTO_OFF") AND NOT ("${parent_status}" STREQUAL "OFF"))
  CHLIB_ADD_SUBSUBSYSTEM(${_parent} ${_name} ${_desc})
endmacro(CHLIB_SUBSUBSYS_OPTION)
...
macro(CHLIB_SUBSYS_DEPEND _var _name)
    set(options)
    set(oneValueArgs)
    set(multiValueArgs DEPS EXT_DEPS OPT_DEPS)
    cmake_parse_arguments(SUBSYS "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN} )
    if(SUBSYS_DEPS)
        SET_IN_GLOBAL_MAP(CHLIB_SUBSYS_DEPS ${_name} "${SUBSYS_DEPS}")
    endif(SUBSYS_DEPS)
    if(SUBSYS_EXT_DEPS)
        SET_IN_GLOBAL_MAP(CHLIB_SUBSYS_EXT_DEPS ${_name} "${SUBSYS_EXT_DEPS}")
    endif(SUBSYS_EXT_DEPS)
    if(SUBSYS_OPT_DEPS)
        SET_IN_GLOBAL_MAP(CHLIB_SUBSYS_OPT_DEPS ${_name} "${SUBSYS_OPT_DEPS}")
    endif(SUBSYS_OPT_DEPS)
    GET_IN_MAP(subsys_status CHLIB_SUBSYS_HYPERSTATUS ${_name})
    if(${_var} AND (NOT ("${subsys_status}" STREQUAL "AUTO_OFF")))
        if(SUBSYS_DEPS)
        foreach(_dep ${SUBSYS_DEPS})
            CHLIB_GET_SUBSYS_STATUS(_status ${_dep})
            if(NOT _status)
                set(${_var} FALSE)
                CHLIB_SET_SUBSYS_STATUS(${_name} FALSE "Requires ${_dep}.")
            else(NOT _status)
                CHLIB_GET_SUBSYS_INCLUDE_DIR(_include_dir ${_dep})
...
# ARGN The subsystems and external libraries to depend on.
macro(CHLIB_SUBSUBSYS_DEPEND _var _parent _name)
    set(options)
    set(parentArg)
    set(nameArg)
    set(multiValueArgs DEPS EXT_DEPS OPT_DEPS)
    ...
    if(SUBSUBSYS_DEPS)
        SET_IN_GLOBAL_MAP(CHLIB_SUBSYS_DEPS ${_parent}_${_name} "${SUBSUBSYS_DEPS}")
    endif(SUBSUBSYS_DEPS)
    if(SUBSUBSYS_EXT_DEPS)
        SET_IN_GLOBAL_MAP(CHLIB_SUBSYS_EXT_DEPS ${_parent}_${_name} "${SUBSUBSYS_EXT_DEPS}")
    endif(SUBSUBSYS_EXT_DEPS)
    if(SUBSUBSYS_OPT_DEPS)
        SET_IN_GLOBAL_MAP(CHLIB_SUBSYS_OPT_DEPS ${_parent}_${_name} "${SUBSUBSYS_OPT_DEPS}")
    endif(SUBSUBSYS_OPT_DEPS)
    GET_IN_MAP(subsys_status CHLIB_SUBSYS_HYPERSTATUS ${_parent}_${_name})
    if(${_var} AND (NOT ("${subsys_status}" STREQUAL "AUTO_OFF")))
        if(SUBSUBSYS_DEPS)
        foreach(_dep ${SUBSUBSYS_DEPS})
            ...
            if(NOT _status)
                set(${_var} FALSE)
                CHLIB_SET_SUBSYS_STATUS(${_parent}_${_name} FALSE "Requires ${_dep}.")
            else(NOT _status)
                CHLIB_GET_SUBSYS_INCLUDE_DIR(_include_dir ${_dep})
        ...
        foreach(_dep ${SUBSUBSYS_EXT_DEPS})
            ...
            if(NOT ${EXT_DEP_FOUND} OR (NOT ("${EXT_DEP_FOUND}" STREQUAL "TRUE")))
                set(${_var} FALSE)
                CHLIB_SET_SUBSYS_STATUS(${_parent}_${_name} FALSE "Requires external library ${_dep}.")
            endif(NOT ${EXT_DEP_FOUND} OR (NOT ("${EXT_DEP_FOUND}" STREQUAL "TRUE")))
        endforeach(_dep)
        endif(SUBSUBSYS_EXT_DEPS)
    endif(${_var} AND (NOT ("${subsys_status}" STREQUAL "AUTO_OFF")))
endmacro(CHLIB_SUBSUBSYS_DEPEND)

###############################################################################
# Add a set of include files to install.
# _component The part of CHLIB that the install files belong to.
# _subdir The sub-directory for these include files.
# ARGN The include files.
macro(CHLIB_ADD_INCLUDES _component _subdir)
    install(FILES ${ARGN} DESTINATION ${INCLUDE_INSTALL_DIR}/${_subdir}
        COMPONENT chlib_${_component})
endmacro(CHLIB_ADD_INCLUDES)


###############################################################################
# Add a library target.
# _name The library name.
# _component The part of CHLIB that this library belongs to.
# 
# ARGN:
#   FILES       The source files for the library.
#   LINK_WITH   List if library to link against
macro(CHLIB_ADD_LIBRARY _name _component)
  # parse arguments
  set(options)
  set(oneValueArgs)
  set(multiValueArgs FILES LINK_WITH)
  cmake_parse_arguments(CHLIB_ADD_LIBRARY "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN} )
  # Create libbrary
  add_library(${_name} ${CHLIB_LIB_TYPE} ${CHLIB_ADD_LIBRARY_FILES})
  # Add link
  TARGET_LINK_LIBRARIES(${_name} ${CHLIB_ADD_LIBRARY_LINK_WITH})
  ...
  set_target_properties(${_name} PROPERTIES 
    VERSION ${CHLIB_VERSION} 
    SOVERSION ${CHLIB_MAJOR_VERSION}.${CHLIB_MINOR_VERSION})

  install(TARGETS ${_name}
    RUNTIME DESTINATION ${BIN_INSTALL_DIR} COMPONENT chlib_${_component}
    LIBRARY DESTINATION ${LIB_INSTALL_DIR} COMPONENT chlib_${_component}
    ARCHIVE DESTINATION ${LIB_INSTALL_DIR} COMPONENT chlib_${_component})
endmacro(CHLIB_ADD_LIBRARY)
...
macro(CHLIB_CUDA_ADD_LIBRARY _name _component)
    REMOVE_VTK_DEFINITIONS()
    if(CHLIB_SHARED_LIBS)
      # to overcome a limitation in cuda_add_library, we add manually CHLIBAPI_EXPORTS macro
      cuda_add_library(${_name} ${CHLIB_LIB_TYPE} ${ARGN} OPTIONS -DCHLIBAPI_EXPORTS)
    else(CHLIB_SHARED_LIBS)
      cuda_add_library(${_name} ${CHLIB_LIB_TYPE} ${ARGN})
    ...
    set_target_properties(${_name} PROPERTIES
      VERSION ${CHLIB_VERSION}
      SOVERSION ${CHLIB_MAJOR_VERSION})

    install(TARGETS ${_name}
        RUNTIME DESTINATION ${BIN_INSTALL_DIR} COMPONENT chlib_${_component}
        LIBRARY DESTINATION ${LIB_INSTALL_DIR} COMPONENT chlib_${_component}
        ARCHIVE DESTINATION ${LIB_INSTALL_DIR} COMPONENT chlib_${_component})
endmacro(CHLIB_CUDA_ADD_LIBRARY)
...
# _component The part of CHLIB that this library belongs to.
# ARGN the source files for the library.
macro(CHLIB_ADD_EXECUTABLE _name _component)
  ...
  set(CHLIB_EXECUTABLES ${CHLIB_EXECUTABLES} ${_name})
  install(TARGETS ${_name} RUNTIME DESTINATION ${BIN_INSTALL_DIR}
      COMPONENT chlib_${_component})
endmacro(CHLIB_ADD_EXECUTABLE)
...
# _bundle 
# ARGN the source files for the library.
macro(CHLIB_ADD_EXECUTABLE_OPT_BUNDLE _name _component)
...
#                         POST_BUILD
#                         COMMAND ${CMAKE_COMMAND} -E create_symlink ${CHLIB_OUTPUT_BIN_DIR}/${_name}.app/Contents/MacOS/${_name} ${CHLIB_OUTPUT_BIN_DIR}/${_name}
# #			WORKING_DIRECTORY 
#                         COMMENT "Creating an alias for ${_name}.app to ${_name}")
    install(TARGETS ${_name} BUNDLE DESTINATION ${BIN_INSTALL_DIR} COMPONENT chlib_${_component})
else(APPLE AND VTK_USE_COCOA)
    install(TARGETS ${_name} RUNTIME DESTINATION ${BIN_INSTALL_DIR} COMPONENT chlib_${_component})
endif(APPLE AND VTK_USE_COCOA)
endmacro(CHLIB_ADD_EXECUTABLE_OPT_BUNDLE)
...
# _component The part of CHLIB that this library belongs to.
# ARGN the source files for the library.
macro(CHLIB_CUDA_ADD_EXECUTABLE _name _component)
  # Add executable
  ...
  install(TARGETS ${_name} RUNTIME DESTINATION ${BIN_INSTALL_DIR}
      COMPONENT chlib_${_component})
endmacro(CHLIB_CUDA_ADD_EXECUTABLE)
...
macro(CHLIB_ADD_TEST _name _exename)
    set(options)
    set(oneValueArgs)
    set(multiValueArgs FILES ARGUMENTS WORKING_DIR LINK_WITH)
    cmake_parse_arguments(CHLIB_ADD_TEST "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN} )
    add_executable(${_exename} ${CHLIB_ADD_TEST_FILES})
    # Add google test framework if not already build
    IF(NOT TARGET gtest)
      ADD_SUBDIRECTORY(${CHLIB_SOURCE_DIR}/3rdparty/googletest ${CHLIB_OUTPUT_3RDPARTY_LIB_DIR}/googletest)
    ...
    include_directories(${CHLIB_SOURCE_DIR}/3rdparty/googletest/googletest/include)
    include_directories(${CHLIB_SOURCE_DIR}/3rdparty/googletest/googlemock/include)
    # Link extra library
    target_link_libraries(${_exename} ${CHLIB_ADD_TEST_LINK_WITH} gtest gmock ${CLANG_LIBRARIES})
    # Only link if needed
    ...
    # Register new text
    add_test(NAME ${_name} COMMAND ${_exename} ${CHLIB_ADD_TEST_ARGUMENTS} WORKING_DIRECTORY ${CHLIB_ADD_TEST_WORKING_DIR})
endmacro(CHLIB_ADD_TEST)
...
macro(CHLIB_ADD_EXAMPLE _name)
    set(options)
    set(oneValueArgs)
    set(multiValueArgs FILES LINK_WITH)
    cmake_parse_arguments(CHLIB_ADD_EXAMPLE "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN} )
    add_executable(${_name} ${CHLIB_ADD_EXAMPLE_FILES})
    target_link_libraries(${_name} ${CHLIB_ADD_EXAMPLE_LINK_WITH} ${CLANG_LIBRARIES})
    if(WIN32 AND MSVC)
      set_target_properties(${_name} PROPERTIES DEBUG_OUTPUT_NAME ${_name}${CMAKE_DEBUG_POSTFIX}
                                                RELEASE_OUTPUT_NAME ${_name}${CMAKE_RELEASE_POSTFIX})
    endif(WIN32 AND MSVC)
endmacro(CHLIB_ADD_EXAMPLE)
...
macro(CHLIB_ADD_CFLAGS _name _flags)
...
macro(CHLIB_ADD_LINKFLAGS _name _flags)
...
endmacro(CHLIB_ADD_LINKFLAGS)


###############################################################################
# Make a pkg-config file for a library. Do not include general CHLIB stuff in the
# arguments; they will be added automaticaly.
# _name The library name. "chlib_" will be preprended to this.
# _component The part of CHLIB that this pkg-config file belongs to.
# _desc Description of the library.
# _chlib_deps External dependencies to chlib libs, as a list. (will get mangled to external pkg-config name)
...
macro(CHLIB_MAKE_PKGCONFIG _name _component _desc _chlib_deps _ext_deps _int_deps _cflags
        _lib_flags)
    set(PKG_NAME ${_name})
    set(PKG_DESC ${_desc})
    ...
    foreach(_dep ${_chlib_deps})
      set(PKG_EXTERNAL_DEPS "${PKG_EXTERNAL_DEPS} chlib_${_dep}-${CHLIB_MAJOR_VERSION}.${CHLIB_MINOR_VERSION}")
    endforeach(_dep)
...
# _int_deps Internal dependencies, as a list.
# _cflags Compiler flags necessary to build with the library.
macro(CHLIB_MAKE_PKGCONFIG_HEADER_ONLY _name _component _desc _chlib_deps _ext_deps _int_deps _cflags)
set(PKG_NAME ${_name})
set(PKG_DESC ${_desc})
set(PKG_CFLAGS ${_cflags})
#set(PKG_LIBFLAGS ${_lib_flags})
...
install(FILES ${_pc_file} DESTINATION ${PKGCFG_INSTALL_DIR}
COMPONENT chlib_${_component})
endmacro(CHLIB_MAKE_PKGCONFIG_HEADER_ONLY)
...
macro(CHLIB_RESET_MAPS)
    foreach(_ss ${CHLIB_SUBSYSTEMS})
        string(TOUPPER "CHLIB_${_ss}_SUBSYS" CHLIB_SUBSYS_SUBSYS)
	if (${CHLIB_SUBSYS_SUBSYS})
            string(TOUPPER "CHLIB_${_ss}_SUBSYS_DESC" CHLIB_PARENT_SUBSYS_DESC)
	    set(${CHLIB_SUBSYS_SUBSYS_DESC} "" CACHE INTERNAL "" FORCE)
	    set(${CHLIB_SUBSYS_SUBSYS} "" CACHE INTERNAL "" FORCE)
	endif (${CHLIB_SUBSYS_SUBSYS})
    endforeach(_ss)

    set(CHLIB_SUBSYS_HYPERSTATUS "" CACHE INTERNAL
        "To Build Or Not To Build, That Is The Question." FORCE)
    set(CHLIB_SUBSYS_STATUS "" CACHE INTERNAL
        "To build or not to build, that is the question." FORCE)
    set(CHLIB_SUBSYS_REASONS "" CACHE INTERNAL
        "But why?" FORCE)
    set(CHLIB_SUBSYS_DEPS "" CACHE INTERNAL "A depends on B and C." FORCE)
    set(CHLIB_SUBSYS_EXT_DEPS "" CACHE INTERNAL "A depends on B and C." FORCE)
    set(CHLIB_SUBSYS_OPT_DEPS "" CACHE INTERNAL "A depends on B and C." FORCE)
    set(CHLIB_SUBSYSTEMS "" CACHE INTERNAL "Internal list of subsystems" FORCE)
    set(CHLIB_SUBSYS_DESC "" CACHE INTERNAL "Subsystem descriptions" FORCE)
endmacro(CHLIB_RESET_MAPS)
...
macro(CHLIB_ADD_SUBSYSTEM _name _desc)
    set(_temp ${CHLIB_SUBSYSTEMS})
    list(APPEND _temp ${_name})
    set(CHLIB_SUBSYSTEMS ${_temp} CACHE INTERNAL "Internal list of subsystems"
        FORCE)
    SET_IN_GLOBAL_MAP(CHLIB_SUBSYS_DESC ${_name} ${_desc})
endmacro(CHLIB_ADD_SUBSYSTEM)

###############################################################################
# Register a subsubsystem.
# _name Subsystem name.
# _desc Description of the subsystem
macro(CHLIB_ADD_SUBSUBSYSTEM _parent _name _desc)
  string(TOUPPER "CHLIB_${_parent}_SUBSYS" CHLIB_PARENT_SUBSYS)
  string(TOUPPER "CHLIB_${_parent}_SUBSYS_DESC" CHLIB_PARENT_SUBSYS_DESC)
  set(_temp ${${CHLIB_PARENT_SUBSYS}})
  list(APPEND _temp ${_name})
  set(${CHLIB_PARENT_SUBSYS} ${_temp} CACHE INTERNAL "Internal list of ${_parenr} subsystems"
    FORCE)
  set_in_global_map(${CHLIB_PARENT_SUBSYS_DESC} ${_name} ${_desc})
endmacro(CHLIB_ADD_SUBSUBSYSTEM)
...
macro(CHLIB_SET_SUBSYS_STATUS _name _status)
    if(${ARGC} EQUAL 3)
        set(_reason ${ARGV2})
    else(${ARGC} EQUAL 3)
        set(_reason "No reason")
    endif(${ARGC} EQUAL 3)
    SET_IN_GLOBAL_MAP(CHLIB_SUBSYS_STATUS ${_name} ${_status})
    SET_IN_GLOBAL_MAP(CHLIB_SUBSYS_REASONS ${_name} ${_reason})
endmacro(CHLIB_SET_SUBSYS_STATUS)
...
# _status TRUE if being built, FALSE otherwise.
# ARGN[0] Reason for not building.
macro(CHLIB_SET_SUBSUBSYS_STATUS _parent _name _status)
    if(${ARGC} EQUAL 4)
        set(_reason ${ARGV2})
    else(${ARGC} EQUAL 4)
        set(_reason "No reason")
    endif(${ARGC} EQUAL 4)
    SET_IN_GLOBAL_MAP(CHLIB_SUBSYS_STATUS ${_parent}_${_name} ${_status})
    SET_IN_GLOBAL_MAP(CHLIB_SUBSYS_REASONS ${_parent}_${_name} ${_reason})
endmacro(CHLIB_SET_SUBSUBSYS_STATUS)
...
macro(CHLIB_GET_SUBSYS_STATUS _var _name)
    GET_IN_MAP(${_var} CHLIB_SUBSYS_STATUS ${_name})
endmacro(CHLIB_GET_SUBSYS_STATUS)

###############################################################################
# Get the status of a subsystem
# _var Destination variable.
# _name Name of the subsystem.
macro(CHLIB_GET_SUBSUBSYS_STATUS _var _parent _name)
    GET_IN_MAP(${_var} CHLIB_SUBSYS_STATUS ${_parent}_${_name})
endmacro(CHLIB_GET_SUBSUBSYS_STATUS)
...
# ARGN[0] Reason for not building.
macro(CHLIB_SET_SUBSYS_HYPERSTATUS _name _dependee _status) 
    SET_IN_GLOBAL_MAP(CHLIB_SUBSYS_HYPERSTATUS ${_name}_${_dependee} ${_status})
    if(${ARGC} EQUAL 4)
        SET_IN_GLOBAL_MAP(CHLIB_SUBSYS_REASONS ${_dependee} ${ARGV3})
    endif(${ARGC} EQUAL 4)
endmacro(CHLIB_SET_SUBSYS_HYPERSTATUS)
...
# ARGN[0] Reason for not building.
macro(CHLIB_GET_SUBSYS_HYPERSTATUS _var _name)
    set(${_var} "AUTO_ON")
    if(${ARGC} EQUAL 3)
        GET_IN_MAP(${_var} CHLIB_SUBSYS_HYPERSTATUS ${_name}_${ARGV2})
    else(${ARGC} EQUAL 3)
        foreach(subsys ${CHLIB_SUBSYS_DEPS_${_name}})
            if("${CHLIB_SUBSYS_HYPERSTATUS_${subsys}_${_name}}" STREQUAL "AUTO_OFF")
                set(${_var} "AUTO_OFF")
                break()
            endif("${CHLIB_SUBSYS_HYPERSTATUS_${subsys}_${_name}}" STREQUAL "AUTO_OFF")
        endforeach(subsys)
    endif(${ARGC} EQUAL 3)
endmacro(CHLIB_GET_SUBSYS_HYPERSTATUS)

###############################################################################
# Set the hyperstatus of a subsystem and its dependee
macro(CHLIB_UNSET_SUBSYS_HYPERSTATUS _name _dependee)
    unset(CHLIB_SUBSYS_HYPERSTATUS_${_name}_${dependee})
endmacro(CHLIB_UNSET_SUBSYS_HYPERSTATUS)
...
# ARGN[0] Reason for not building.
macro(CHLIB_SET_SUBSYS_INCLUDE_DIR _name _includedir)
    SET_IN_GLOBAL_MAP(CHLIB_SUBSYS_INCLUDE ${_name} ${_includedir})
endmacro(CHLIB_SET_SUBSYS_INCLUDE_DIR)
...
# _var Destination variable.
# _name Name of the subsystem.
macro(CHLIB_GET_SUBSYS_INCLUDE_DIR _var _name)
    GET_IN_MAP(${_var} CHLIB_SUBSYS_INCLUDE ${_name})
    if(NOT ${_var})
      set (${_var} ${_name})
    endif(NOT ${_var})
endmacro(CHLIB_GET_SUBSYS_INCLUDE_DIR)


###############################################################################
# Write a report on the build/not-build status of the subsystems
macro(CHLIB_WRITE_STATUS_REPORT)
    ...
    foreach(_ss ${CHLIB_SUBSYSTEMS})
		...
		foreach(_sub ${${CHLIB_SUBSYS_SUBSYS}})
		    CHLIB_GET_SUBSYS_STATUS(_sub_status ${_ss}_${_sub})
		    if (_sub_status)
		...
		foreach(_sub ${${CHLIB_SUBSYS_SUBSYS}})
		    CHLIB_GET_SUBSYS_STATUS(_sub_status ${_ss}_${_sub})
		    CHLIB_GET_SUBSYS_HYPERSTATUS(_sub_hyper_status ${_ss}_${sub})
		    if (NOT _sub_status OR ("${_sub_hyper_status}" STREQUAL "AUTO_OFF"))
		        GET_IN_MAP(_reason CHLIB_SUBSYS_REASONS ${_ss}_${_sub})
...
macro(collect_subproject_directory_names dirname filename names dirs)
    ...
    foreach(subdir ${${dirs}})
        ...
        if(NOT("${DEPENDENCIES}" STREQUAL ""))
            ...
            string(TOUPPER "CHLIB_${name}_DEPENDS" SUBSYS_DEPENDS)
            set(${SUBSYS_DEPENDS} ${DEPENDENCIES})
            foreach(dependee ${DEPENDENCIES})
                string(TOUPPER "CHLIB_${dependee}_DEPENDIES" SUBSYS_DEPENDIES)
...
macro(CHLIB_DISABLE_DEPENDIES _subsys)
    string(TOUPPER "chlib_${_subsys}_dependies" CHLIB_SUBSYS_DEPENDIES)
    if(NOT ("${${CHLIB_SUBSYS_DEPENDIES}}" STREQUAL ""))
        foreach(dep ${${CHLIB_SUBSYS_DEPENDIES}})
            CHLIB_SET_SUBSYS_HYPERSTATUS(${_subsys} ${dep} AUTO_OFF "Disabled: ${_subsys} missing.")
            set(BUILD_${dep} OFF CACHE BOOL "Disabled: ${_subsys} missing." FORCE)
        endforeach(dep)
    endif(NOT ("${${CHLIB_SUBSYS_DEPENDIES}}" STREQUAL ""))
endmacro(CHLIB_DISABLE_DEPENDIES subsys)

########################################################################################
# Macro to enable subsystem dependies
# _subsys IN subsystem name
macro(CHLIB_ENABLE_DEPENDIES _subsys)
    string(TOUPPER "chlib_${_subsys}_dependies" CHLIB_SUBSYS_DEPENDIES)
    if(NOT ("${${CHLIB_SUBSYS_DEPENDIES}}" STREQUAL ""))
        foreach(dep ${${CHLIB_SUBSYS_DEPENDIES}})
            CHLIB_GET_SUBSYS_HYPERSTATUS(dependee_status ${_subsys} ${dep})
            if("${dependee_status}" STREQUAL "AUTO_OFF")
                CHLIB_SET_SUBSYS_HYPERSTATUS(${_subsys} ${dep} AUTO_ON)
                GET_IN_MAP(desc CHLIB_SUBSYS_DESC ${dep})
                set(BUILD_${dep} ON CACHE BOOL "${desc}" FORCE)
            endif("${dependee_status}" STREQUAL "AUTO_OFF")
        endforeach(dep)
    endif(NOT ("${${CHLIB_SUBSYS_DEPENDIES}}" STREQUAL ""))
endmacro(CHLIB_ENABLE_DEPENDIES subsys)

########################################################################################
# Macro to build subsystem centric documentation
# _subsys IN the name of the subsystem to generate documentation for
macro (CHLIB_ADD_DOC _subsys)
  string(TOUPPER "${_subsys}" SUBSYS)
  set(doc_subsys "doc_${_subsys}")
  GET_IN_MAP(dependencies CHLIB_SUBSYS_DEPS ${_subsys})
  if(DOXYGEN_FOUND)
    ...
    if(NOT "${dependencies}" STREQUAL "")
      set(STRIPPED_HEADERS "${CHLIB_SOURCE_DIR}/${dependencies}/include")
...
macro(CHLIB_ADD_GRABBER_DEPENDENCY _name _description)
...
endmacro(CHLIB_ADD_GRABBER_DEPENDENCY)
