...
#ifndef __OGLKIT_quaternion__
#define __OGLKIT_quaternion__
 ...
 */
template <typename T>
class OGLKIT_EXPORTS Quaternion {
 public:
...
#pragma mark Initialization
  ...
  /**
   *  @name Quaternion
   *  @fn Quaternion(const T q, const OGLKit::Vector3<T>& v)
   *  @brief  Constructor
   *  @param[in]  q   Real value to initialize with
   *  @param[in]  v   Imaginary value to initialize with
   */
  Quaternion(const T q, const OGLKit::Vector3<T>& v) : q_(q), v_(v) {
  }
  ...
  /**
   *  @name Quaternion
   *  @fn Quaternion(const OGLKit::Vector3<T>& axis, const T angle)
   *  @brief  Constructor
   *  @param[in]  axis   Rotation axis
   *  @param[in]  angle  Amount of rotation around a given \p axis.
   */
  Quaternion(const OGLKit::Vector3<T>& axis, const T angle) {
    // Define real part
