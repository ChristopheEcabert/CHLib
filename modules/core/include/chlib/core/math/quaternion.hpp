...
#ifndef __CHLIB_quaternion__
#define __CHLIB_quaternion__
...
#pragma mark Initialization
  ...
  /**
   *  @name Quaternion
   *  @fn Quaternion(const T q, const CHLib::Vector3<T>& v)
   *  @brief  Constructor
   *  @param[in]  q   Real value to initialize with
   *  @param[in]  v   Imaginary value to initialize with
   */
  Quaternion(const T q, const CHLib::Vector3<T>& v) : q_(q), v_(v) {
  }
  ...
  /**
   *  @name Quaternion
   *  @fn Quaternion(const CHLib::Vector3<T>& axis, const T angle)
   *  @brief  Constructor
   *  @param[in]  axis   Rotation axis
   *  @param[in]  angle  Amount of rotation around a given \p axis.
   */
  Quaternion(const CHLib::Vector3<T>& axis, const T angle) {
    // Define real part
