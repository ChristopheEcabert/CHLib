...
#ifndef __OGLKIT_AABB__
#define __OGLKIT_AABB__
...
#pragma mark Usage
   ...
   */
  static T SquaredDistanceToPoint(const OGLKit::Vector3<T>& p,
                                  const AABB<T>& bbox) {
    T sq_dist = T(0.0);
    const T* p_ptr = &p.x_;
   ...
   */
  static bool IntersectObject(const OGLKit::Vector3<T>& p,
                              const OGLKit::Vector3<T>& dir,
                              const AABB<T>& bbox,
                              T* t) {
...
#pragma mark Members
  ...
  OGLKit::Vector3<T> min_;
  /** Maximum corner (i.e. bottom right) */
  OGLKit::Vector3<T> max_;
  /** Index */
  int index_;
  /** Center */
  OGLKit::Vector3<T> center_;


};
#endif /* __OGLKIT_AABB__ */
