...
#ifndef __CHLIB_AABB__
#define __CHLIB_AABB__
...
#pragma mark Usage
   ...
   */
  static T SquaredDistanceToPoint(const CHLib::Vector3<T>& p,
                                  const AABB<T>& bbox) {
    T sq_dist = T(0.0);
    const T* p_ptr = &p.x_;
   ...
   */
  static bool IntersectObject(const CHLib::Vector3<T>& p,
                              const CHLib::Vector3<T>& dir,
                              const AABB<T>& bbox,
                              T* t) {
...
#pragma mark Members
  ...
  CHLib::Vector3<T> min_;
  /** Maximum corner (i.e. bottom right) */
  CHLib::Vector3<T> max_;
  /** Index */
  int index_;
  /** Center */
  CHLib::Vector3<T> center_;


};
#endif /* __CHLIB_AABB__ */
